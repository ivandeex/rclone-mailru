	...
	Run: func(command *cobra.Command, args []string) {
		...
		cmd.Run(false, false, command, func() error {
			...
			if hashsum.HashsumOutfile == "" {
				return operations.HashLister(ctx, hash.MD5, hashsum.OutputBase64, hashsum.DownloadFlag, fsrc, nil, nil)
			}
			...
			defer close()
			err = operations.HashLister(ctx, hash.MD5, hashsum.OutputBase64, hashsum.DownloadFlag, fsrc, output, hashBuf)
			if err == nil && hashBuf != nil {
				err = hashsum.SortOutputFile(output, hashBuf, hash.MD5)
			}
			return err
		})
	},
